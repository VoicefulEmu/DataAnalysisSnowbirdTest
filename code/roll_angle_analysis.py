...
from path_config import default_analysis_dir, default_hr_csv, ensure_standard_dirs
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Integrate HR Gyro_X into cumulative roll angle for ascent/descent plots."
    )
    parser.add_argument(
        "--hr-csv",
        default=None,
    ...
    parser.add_argument(
        "--outdir",
        default=None,
        help="Optional output directory. Defaults to exports/<hr_stem>_analysis.",
    )
    parser.add_argument(
        "--show",
        action="store_true",
    ...
    csv_path = Path(args.hr_csv) if args.hr_csv else default_hr_csv()
    ...
    print(f"Roll angle at apogee   : {ang_asc[-1]:.1f} deg")
    print(f"Roll angle at end      : {ang_desc[-1]:.1f} deg")
    print(f"Total roll (flight)    : {roll_angle[-1]:.1f} deg")
...
# ── Plot helper ────────────────────────────────────────────────────────────────
    def make_roll_plot(t_seg, ang_seg, phase_label, event_time=None, event_label=None,
        ...
        if angle_at_phase_start is not None:
            ax.axhline(angle_at_phase_start, color="grey", lw=0.8, ls=":",
                       label=f"Angle at phase start ({angle_at_phase_start:.0f} deg)")
        ...
        ax2.set_ylabel("Roll Rate (deg/s)", fontsize=11)
    ...
    fig_asc = make_roll_plot(
        t_asc, ang_asc,
        phase_label="Ascent (launch to apogee)",
    ...
    fig_desc = make_roll_plot(
        t_desc, ang_desc,
        phase_label="Descent (apogee to landing)",
        event_time=MAIN_TIME_S,
        event_label=f"Main chute fire ({MAIN_TIME_S} s)",
        angle_at_phase_start=ang_asc[-1],
    )

    out_dir = Path(args.outdir) if args.outdir else default_analysis_dir(csv_path)
