...
from path_config import PHOTOS_DIR, default_hr_csv, default_lr_csv, ensure_standard_dirs


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare HR acceleration with LR-derived acceleration and save a summary plot."
    )
    parser.add_argument(
        "--hr-csv",
        default=None,
        help="Optional path to HR CSV. Defaults to latest HR CSV in exports/.",
    )
    parser.add_argument(
        "--lr-csv",
        default=None,
        help="Optional path to LR CSV. Defaults to latest LR CSV in exports/.",
    )
    parser.add_argument(
        "--out",
        default=None,
        help="Optional output PNG path. Defaults to photos/acceleration_vs_time_analysis.png.",
    )
    parser.add_argument(
        "--show",
        action="store_true",
    ...
    hr_file = Path(args.hr_csv) if args.hr_csv else default_hr_csv()
    lr_file = Path(args.lr_csv) if args.lr_csv else default_lr_csv()
    out_file = Path(args.out) if args.out else PHOTOS_DIR / "acceleration_vs_time_analysis.png"
    ...
    print()
    idx_219 = int(np.argmin(np.abs(lr_data['Flight_Time_(s)'].to_numpy(dtype=float) - 219.36)))
    print(f"Altitude at time 219.36s (parachute fires):  {lr_data.iloc[idx_219]['Baro_Altitude_AGL_(feet)']:.1f} ft")
