...
from path_config import PHOTOS_DIR, default_eng_file, default_hr_csv, ensure_standard_dirs


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Overlay AeroTech motor thrust and Blue Raven HR accel magnitude (0-4s)."
    )
    parser.add_argument(
        "--eng-file",
        default=None,
        help="Path to .eng file. Defaults to code/AeroTech_O5500X-PS.eng.",
    )
    parser.add_argument(
        "--hr-csv",
        default=None,
        help="Path to HR CSV. Defaults to latest HR CSV in exports/.",
    )
    parser.add_argument(
        "--out",
        default=None,
        help="Output PNG path. Defaults to photos/aerotech_vs_blueraven_0_4s.png.",
    )
    parser.add_argument("--show", action="store_true", help="Display plot after saving.")
    args = parser.parse_args()

    ensure_standard_dirs()
    aerotech_file = Path(args.eng_file) if args.eng_file else default_eng_file()
    blueraven_hr_file = Path(args.hr_csv) if args.hr_csv else default_hr_csv()
    out_file = Path(args.out) if args.out else PHOTOS_DIR / "aerotech_vs_blueraven_0_4s.png"
